...
type FileItem = {
  ...
  dateLabel: string;
  sizeLabel: string;
  folderId?: string; // important for "inside folder"
};
...
export default function ClientDocumentsPage() {
  ...
  // Folder navigation: null means main documents page, else we're "inside" folder
  ...
  // menus
  const [openMenuId, setOpenMenuId] = useState<string | null>(null); // file menu id
  const [openFolderMenuId, setOpenFolderMenuId] = useState<string | null>(null); // folder menu id
  ...
  const scopedFiles = useMemo(() => {
    // main view = all files, folder view = only that folder
    ...
    <div className={styles.page} onClick={closeAllPopups}>
      <div className={styles.headerRow} onClick={(e) => e.stopPropagation()}>
        <div className={styles.titleWrap}>
          ...
          {/* Breadcrumb only when inside folder */}
          {activeFolder && (
            <div className={styles.breadcrumb}>
              <button className={styles.breadcrumbLink} onClick={goBackToRoot} type="button">
                Folders
              </button>
          ...
          <div className={styles.ddWrap}>
            ...
            {newOpen && (
              <div className={styles.menu}>
                ...
                <button className={styles.menuItem} onClick={() => console.log("folder upload")}>
                  <Folder className={styles.menuIcon} />
                  Folder Upload
                </button>
              </div>
      ...
      {/* Recent: show on root + on folder (folder-scoped recent) */}
      <section className={styles.section}>
        ...
        <div className={styles.table}>
          ...
          {filteredFiles.map((f) => {
            ...
            return (
              <div
                key={f.id}
                className={cn(styles.tableRow, checked && styles.rowSelected)}
              >
                <div className={styles.cellCheck}>
                  <input
                    type="checkbox"
