...
export default function AdminDocuments() {
 ...
 async function actionDownloadFile(file: FileItem) {
   try {
     if (file.contentType === "text/html") {
       ...
       pushToast("Exported formatted PDF.", "success")
       setOpenMenuKey(null)
       return
     }
     ...
     pushToast(error?.message ?? "Export failed.", "danger")
   }
 }
 ...
 async function submitMoveDocument() {
   ...
   try {
     ...
     await moveDocument(moveTargetId, nextFolderId)

     setFilesAll((prev) =>
       prev.map((file) =>
         file.id === moveTargetId
           ? {
               ...file,
     ...
     setMoveTargetId("")
     setMoveFolderId("")
     pushToast("Document moved.", "success")
   } catch (error: any) {
     pushToast(error?.message ?? "Move failed.", "danger")
   }
 }
 ...
 return (
   ...
   >
       ...
       <div className="space-y-3">
         ...
         {viewFile?.contentType === "text/html" ? (
           <div className="h-[72vh] overflow-hidden rounded-xl border border-gray-200 bg-gray-100 shadow-inner dark:border-slate-700 dark:bg-slate-950/40">
             <iframe
               title={viewFile?.name ?? "Document Preview"}
               srcDoc={viewFile?.content || "<p>No content saved for this document.</p>"}
     ...
     <Modal open={moveOpen} title="Move Document" onClose={() => setMoveOpen(false)}>
       <div className="space-y-3">
         <div className="text-sm text-gray-600 dark:text-slate-300">
           Choose where this document should be stored.
         </div>
         ...
         <div className="space-y-2">
           ...
           >
             ...
             {folders
               .filter((folder) => !archivedFolderIds[folder.id])
               .map((folder) => (
         ...
         <div className="flex justify-end gap-2">
           <button className={btnBase} type="button" onClick={() => setMoveOpen(false)}>
             Cancel
           </button>
           <button className={btnPrimary} type="button" onClick={submitMoveDocument}>
             Move
           </button>
         </div>
       </div>
